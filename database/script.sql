...
-- lay thong tin mon thi
CREATE PROCEDURE SP_GET_EXAM_INFO (@courseId VARCHAR(20))
AS
BEGIN
	SELECT DT.NGAY_THI, DT.TG_THI, MH.TEN_MH, DT.ID_MA_DE
	FROM dbo.DE_THI DT JOIN dbo.MON_HOC MH ON MH.MA_MH = DT.MA_MH
	WHERE MH.MA_MH = @courseId;
END
GO

-- lay goi cau hoi de thi
CREATE PROCEDURE SP_GET_QUESTIONS (@examId INT)
AS
BEGIN
	SELECT ID_CH, STT, CAU_HOI, CAU_TL_1, CAU_TL_2, CAU_TL_3, CAU_TL_4 
	FROM dbo.CAU_HOI 
	WHERE ID_MA_DE = @examId;
END
GO
